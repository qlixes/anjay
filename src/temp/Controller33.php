...
class Controller extends Common
{
	...
	function add_login()
	{
		...
		{
			$this->data['user_name'] = $this->session('user_name');
			$this->data['list_module'] = $this->pdqa_login_module();
			...
			if(($this->input2('save_add_login') !== null))
			{
				...
				if(!empty($check))
				{
					...
					if($user_status) //dipisah utk dptkan user_id
					{
						foreach($this->data['list_module'] as $value)
						{
							$modules['user_id'] = $user_id;
							$modules['module_id'] = $value['id'];

							$checked = (!empty($this->input2('list_check'))) ? $this->input2('list_check') : $this->data['list_module'];
						
							$modules['is_active'] = in_array($value['module_name'], $checked)  ? 1 : 0;

							list($module_status, $module_id) = $this->pdqa_model->add_user_module($modules, $this->request('id'));
						}
						var_dump('lanjutan jika sukses save tbl_user');
					}
	...
	function show_login()
	{
		...
		{
			$this->data['user_name'] = $this->session('user_name');
			$this->data['data_login'] = $this->pdqa_login_search();
	...
	function pdqa_login_search()
	{
		$model = $this->pdqa_model->get_user(array(
			'user_names'	=>	$this->input2('user_names'),
		));
		return $model[1];
	}
